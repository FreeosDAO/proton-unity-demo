...
async function _GetProtonAvatar(account) {
 try {
...
async function _CreateLink(restoreSession = false) {
 const {link: link, session: session} = await window.ProtonWebSDK({
  linkOptions: {
...
async function _Reconnect(callback) {
 if (!Proton.session) {
...
async function _Transfer(to, amount, callback) {
 if (!Proton.session) {
...
var asmLibraryArg = {
 "GetProtonAvatar": _GetProtonAvatar,
...
if (!Object.getOwnPropertyDescriptor(Module, "Proton")) Module["Proton"] = function() {
 abort("'Proton' was not exported. add it to EXPORTED_RUNTIME_METHODS (see the FAQ)");
};
